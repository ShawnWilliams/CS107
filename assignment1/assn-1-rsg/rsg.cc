...
void generateRandomSentence(map<string, Definition> grammar, string nonterminal);
...
int main(int argc, char *argv[])
{
  ...
  while(curr != grammar.end()){
	...
	def = grammar[curr->first];
	prod = def.getRandomProduction();
  ...
  */
  ...
  for(int i =1; i <= 3; i++){
      cout << "Version #" << i << endl;
      generateRandomSentence(grammar, "<start>");
...
// recursively generate random sentences
void generateRandomSentence(map<string, Definition> grammar, string nonterminal)
{
	Definition def = grammar[nonterminal];
	Production prod = def.getRandomProduction();
	for (Production::iterator prodIter = prod.begin(); prodIter != prod.end(); prodIter++){
		string curstring = *prodIter;

		if(curstring[0] == '<' && curstring[curstring.size()-1] == '>'){
			generateRandomSentence(grammar, curstring);
		}
